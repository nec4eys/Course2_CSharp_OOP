...
public class Matrix
{
    ...
    public static Matrix GetProduct(Matrix matrix1, Matrix matrix2)
    {
        ...
        double[,] resultMatrixArray = new double[matrix1.RowsCount, matrix2.ColumnsCount];

        for (int i = 0; i < matrix1.RowsCount; i++)
        {
            for (int j = 0; j < matrix2.ColumnsCount; j++)
            {
                resultMatrixArray[i, j] = Vector.GetScalarProduct(matrix1._rows[i], matrix2.GetColumnByIndex(j));
            }
        }
